...
function App() {
  const [userAuthentication, setUserAuthentication] = useState({ login: false })
  useEffect(() => {
    (async () => {
      const authenticationStatus = await axios.get("http://localhost:8000/authentication/check-authentication", { withCredentials: true })
      if (authenticationStatus.data.login) {
        console.log(authenticationStatus.data)
        setUserAuthentication(authenticationStatus.data)
